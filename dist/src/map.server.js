...
class MapServer {
    ...
    loopPhysics() {
        ...
        for (let i = 0; i < globalThis.allSockets.length; i++) {
            let session = globalThis.allSockets[i];
            const headingVector = new open3d_1.Vector3d(session.actor.headingX, session.actor.headingY, session.actor.headingZ);
            const poisitionPoint = new open3d_1.Point3d(session.actor.posX, session.actor.posY, session.actor.posZ);
            const velocityVector = headingVector.Multiply(session.actor.speed * Math.min(deltaTime, session.actor.duration));
            const translation = open3d_1.Transform.Translation(velocityVector);
    ...
    loopCommands() {
        globalThis.commandQueue.forEach((command) => this.processCommand(command));
    ...
    processCommand(command) {
        ...
        switch (command.cmd) {
            case 'RemoveSession':
                globalThis.allSockets.forEach((s) => {
